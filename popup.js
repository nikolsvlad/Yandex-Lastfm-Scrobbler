...
$('btnGetToken').addEventListener('click', async () => {
  ...
  if (!apiKey || !apiSecret) {
    showError('setupError', 'Введите API Key и Shared Secret');
    return;
  }
  if (apiKey.length < 20 || apiSecret.length < 20) {
    showError('setupError', 'Похоже, ключи неверные. Проверьте их на last.fm/api');
    return;
  }

  $('btnGetToken').disabled = true;
  $('btnGetToken').textContent = 'Получаем токен...';

  try {
    const result = await sendBg({ type: 'AUTH_GET_TOKEN', apiKey, apiSecret });
    if (result.error) throw new Error(result.error);

    // Открываем страницу авторизации Last.fm
    ...
    $('btnGetToken').textContent = 'Продолжить →';
  }
});
