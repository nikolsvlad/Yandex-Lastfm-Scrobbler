...
const LASTFM_API_KEY = ''; // Пользователь вводит в popup
const LASTFM_API_SECRET = ''; // Пользователь вводит в popup
...
async function handleMessage(msg, sender) {
  ...
  switch (msg.type) {
    ...
    case 'AUTH_GET_TOKEN': {
      if (!msg.apiKey || !msg.apiSecret) return { error: 'Нет ключей API' };
      const tempApi = new LastFmAPI(msg.apiKey, msg.apiSecret, null);
      const token = await tempApi.getToken();
      await saveSettings({ pendingToken: token, apiKey: msg.apiKey, apiSecret: msg.apiSecret });
      api = tempApi;
      const authUrl = `https://www.last.fm/api/auth/?api_key=${msg.apiKey}&token=${token}`;
